...
def main():
    ...
    nextseqnum = 1
    highestACKed = 0
    # Send all packets from base to base+N
    # Then wait for acks, the moment u get one, shift base and send remaining packets
    # If timed out, send whole window again
    start_time = time.time()
    while highestACKed < len(packetList):
        while nextseqnum < base + window_size and nextseqnum -1 < len(packetList):
            socket_udp.sendto(packetList[nextseqnum-1], recieverAddressPort)
            print('Sent pkt '+str(nextseqnum))
            nextseqnum+=1
        signal.setitimer(signal.ITIMER_REAL, default_timeout)
        try:
            recvd = socket_udp.recvfrom(packetSize)
            msg = recvd[0]
            ackFlag = msg[6:7].decode()
            seqofAck = msg[0:5].decode()
            if ackFlag == '1':
                if int(seqofAck) <= highestACKed:
                    print('Received ACK for '+seqofAck+', expecting atleast '+str(highestACKed))
                    continue
                else:
                    # Received an ack 
                    print('Received ACK for '+seqofAck)
                    signal.alarm(0)
                    highestACKed = int(seqofAck)
                    base = highestACKed + 1
        except:
            # Timed out => Resend the window
            print('Timed out for pkt '+str(base))
            nextseqnum = base
    end_time = time.time()
    print("Sent!, " + str(highestACKed) + " packets was sent")
